...
- Lifecycle  : draft (5), hidden (5)  -- currently both at 0
- Moderation : rejected (5)
...
with app.app_context():
    ...
    # ── 2. Lifecycle: HIDDEN (need 5 from active pool) ─────────
    current_hidden = Internship.query.filter_by(
        lifecycle_status_id=lc['hidden'].id, deleted_at=None
    ).count()
    ...
    if needed_hidden > 0:
        ...
        ).order_by(Internship.id.asc()).limit(needed_hidden).all()
        for row in candidates:
            row.lifecycle_status_id = lc['hidden'].id
        db.session.flush()
        print(f"[+] Lifecycle HIDDEN : {len(candidates)} lowongan diupdate (total -> {current_hidden + len(candidates)})")
    else:
        print(f"[=] Lifecycle HIDDEN   : sudah ada {current_hidden}, skip.")
